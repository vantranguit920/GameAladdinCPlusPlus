...
void ObjectManager::Init(Graphic* graphic, Sound* sound)
{
	...
	//
	aladin = new Aladdin(spriteAladdin, infoAlddin);
	point = new PointMap(aladin);

	bonusLevel = new BonusLevel(spriteBonusLevel, infoBonusLevel, D3DXVECTOR2(400, 60), aladin);
...
void ObjectManager::Update(float dt, Keyboard* keyboard)
{
	...
	for (size_t i = 0; i < listObjectCollison.size(); i++) {
		...
		listObjectCollison.at(i)->OnCollision(aladin,disMan);
	}
